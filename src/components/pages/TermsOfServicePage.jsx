...
function TocLink({ id, label, isActive }) {
  return (
    <a href={`#${id}`} className={`${styles.tocLink} ${isActive ? styles.active : ''}`}>
      {label}
    </a>
  );
}
...
export function TermsOfServicePage() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const scrollPosition = 200; // Offset for sticky header

      let currentId = headingElements[0] ? headingElements[0].id : '';
  ...
  useEffect(() => {
    ...
    const activeLink = tocListRef.current.querySelector(`a[href="#${activeId}"]`);
    if (activeLink) {
      ...
      // Keep active item in view with some padding
      if (linkTop < containerScroll + 20) {
        container.scrollTo({ top: linkTop - 20, behavior: 'smooth' });
      } else if (linkTop + linkHeight > containerScroll + containerHeight - 20) {
        container.scrollTo({ top: linkTop - containerHeight + linkHeight + 20, behavior: 'smooth' });
      }
    }
  }, [activeId]);
    ...
    <>
      <main>
        ...
        <section className={styles.termsSection} aria-labelledby="terms-document-title">
          <Container>
            <div className={styles.generalDisclaimer} data-reveal>
              <h2>General Disclaimer</h2>
              <ParagraphGroup
                paragraphs={[
                  'Associum.ai provides research assistance, document analysis, and information synthesis for educational and informational purposes only. Associum.ai is not registered as an investment adviser or broker-dealer with the SEC (USA), not authorised by the FCA (UK), and not registered with SEBI (India). Nothing on this platform constitutes financial, legal, tax, or investment advice, nor should any output be construed as a recommendation to buy, sell, or hold any security or asset class or to make any other consequential financial or legal decision. Associum.ai does not provide personalised investment, accounting, tax, or legal advice of any kind.',
                  'The information generated by Associum.ai may be derived from third-party sources, public data, or user-uploaded documents. While we strive for accuracy, Associum.ai makes no representations or warranties express or implied as to the completeness, accuracy, reliability, or timeliness of any content produced. You should not rely on any such content as the sole basis for any business, financial, or legal decision.',
                  'Users should consult a qualified and licensed professional including a financial adviser, attorney, or accountant in your jurisdiction before making any decision that may have legal, regulatory, or financial consequences. Users are responsible for any decisions or recommendations made using outputs from Associum.ai.'
                ]}
              />
            </div>
            ...
            <div className={styles.termsLayout}>
              <aside className={styles.tocCard}>
                ...
                <nav className={styles.tocList} aria-label="Terms of contents" ref={tocListRef}>
                  {tableOfContents.map((item) => (
                    <TocLink key={item.id} id={item.id} label={item.label} isActive={activeId === item.id} />
                  ))}
                </nav>
              </aside>
